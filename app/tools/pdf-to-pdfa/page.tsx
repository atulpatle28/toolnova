...
import { ArrowLeft, Download, ShieldCheck, Shield, RefreshCw, CheckCircle2 } from "lucide-react";
...
function PdfToPdfAPage() {
  ...
  const convertToPdfA = async () => {
    ...
    try {
      ...
      // Fixed: Uint8Array.buffer cast to ArrayBuffer to resolve TypeScript BlobPart error
      ...
      <main className="flex-1 max-w-[1200px] w-full mx-auto p-4 sm:p-6 space-y-6">
        <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-900 border border-slate-800">
          ...
          <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4" /> ToolKraft PDF to PDF/A
          </span>
        </div>

        <div className="text-center space-y-2">
          <h1 className="text-3xl font-extrabold text-white">PDF to PDF/A Converter</h1>
          <p className="text-xs text-slate-400">Convert standard PDF documents into ISO-compliant PDF/A archival format.</p>
        </div>
            ...
            <div className="space-y-6">
              ...
              {!pdfUrl ? (
                ...
                <div className="p-4 bg-emerald-950/40 border border-emerald-800/50 rounded-2xl space-y-3">
                  ...
                  <a
                    href={pdfUrl}
                    download="toolkraft-archival-document.pdf"
